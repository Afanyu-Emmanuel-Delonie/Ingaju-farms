...
const SIZE = 520;
const CENTER = 260;
const NODE_ORBIT_R = 192; // distance from center to each circle's center
const NODE_R = 60; // each photo-circle's own radius
const RING_R = 50; // thin center brand ring
          ...
          {STAGE_ORDER.map((key) => {
            ...
            return (
              <div
                ...
                className={`group absolute z-10 flex aspect-square cursor-pointer flex-col overflow-hidden rounded-full border-2 bg-white p-[8%] transition-colors duration-300 ${
                  isActive ? "border-[#3A7D5A]" : "border-neutral-200 hover:border-[#3A7D5A]/50"
                }`}
              >
                {/* Photo — inset from the circle edge with its own rounded corners */}
                <div className="relative h-[48%] w-full flex-shrink-0 overflow-hidden rounded-2xl">
                ...
                <div
                  ...
                  className="mt-[8%] flex w-full flex-shrink-0 items-center justify-center gap-1.5 rounded-lg bg-[#3A7D5A] py-[6%]"
                >

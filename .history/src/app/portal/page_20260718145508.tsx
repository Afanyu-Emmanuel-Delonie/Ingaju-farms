...
import { ShoppingCart, MessageSquare, FileText, Plus, X, ChevronDown, Clock } from "lucide-react";
...
export default function PortalPage() {
  ...
  const submitOrder = async (e: React.FormEvent) => {
    ...
    if (!db || !isFirebaseConfigured) {
      setSubmitting(false);
      setSuccessMsg("Firebase is currently disabled. Configure your Firebase credentials to enable submissions.");
      return;
    }
  ...
  return (
    <div className="container-pad mx-auto py-10 max-w-5xl">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm font-body text-[#6B6259]">Manage orders, inquiries, and other requests in one place.</p>
      </div>

      {!isFirebaseConfigured && (
        <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-body text-amber-800">
          Firebase is currently disabled. Add your Firebase credentials later to re-enable submissions and authentication.
        </div>
      )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5 sm:col-span-2">
                  ...
                  <div className="relative">
                    <select
                      required value={orderForm.product}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="number" min="1" required value={orderForm.qty}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    type="text" required value={orderForm.unit}
          ...
          {activeTab === "inquiries" && (
            <form onSubmit={submitInquiry} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <select
                    required value={inquiryForm.topic}
                    onChange={(e) => setInquiryForm((f) => ({ ...f, topic: e.target.value }))}
                    className="w-full rounded-xl border border-[#E0D8CE] px-4 py-3 text-sm font-body text-[#1C2321] bg-[#F8F6F2] outline-none focus:border-[#3A7D5A] appearance-none transition-colors"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-body font-semibold uppercase tracking-widest text-[#6B6259]">Your Name</label>
                  <input
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  rows={4} required value={inquiryForm.message}
          ...
          {activeTab === "other" && (
            <form onSubmit={submitOther} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  type="text" required value={otherForm.subject}
                  onChange={(e) => setOtherForm((f) => ({ ...f, subject: e.target.value }))}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  rows={5} required value={otherForm.details}
                  onChange={(e) => setOtherForm((f) => ({ ...f, details: e.target.value }))}
            ...
            <div key={s.id} className="bg-white rounded-2xl border border-neutral-200 p-5 flex flex-col sm:flex-row sm:items-start gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className={`text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${statusBadge(s.type)}`}>
                    {s.type}
                  </span>

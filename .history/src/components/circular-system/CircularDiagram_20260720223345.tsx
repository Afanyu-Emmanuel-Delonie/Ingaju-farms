...
interface StageDetail {
  ...
  angle: number; // 0=right, 90=bottom, 180=left, 270=top (SVG/CSS convention)
}
...
const STAGE_DATA: Record<StageKey, StageDetail> = {
  cattle: {
    label: "Dairy Cattle",
    desc: "Healthy cows produce milk and manure.",
  ...
  fertilizer: {
    ...
    desc: "Manure is processed into natural fertilizer to enrich the soil.",
  ...
  crop: {
    ...
    desc: "Healthy soil grows nutritious crops.",
  ...
  feed: {
    label: "Animal Feed",
    desc: "Crops and crop residues are used as feed for the cattle.",
...
const SIZE = 500;
const CENTER = 250;
const NODE_ORBIT_R = 172; // distance from center to each circle's center
const NODE_R = 70; // each photo-circle's own radius
const RING_R = 80; // thin center brand ring
const ARC_BOW_R = 206; // how far the connecting arcs bow outward
...
// Curved arrow between two adjacent stage circles, bowing outward around the loop
function arcBetween(a1: number, a2: number) {
  const c1 = pointOnCircle(a1, NODE_ORBIT_R);
  const c2 = pointOnCircle(a2, NODE_ORBIT_R);
  const dir = normalize(c2.x - c1.x, c2.y - c1.y);
  const start = { x: c1.x + dir.x * NODE_R, y: c1.y + dir.y * NODE_R };
  const end = { x: c2.x - dir.x * (NODE_R + 14), y: c2.y - dir.y * (NODE_R + 14) };
  let mid = (a1 + a2) / 2;
  if (a2 < a1) mid += 180; // wrap fix for the 270 -> 360(0) edge
  const ctrl = pointOnCircle(mid, ARC_BOW_R);
        ...
        >
          ...
          >
            <defs>
              ...
              >
                <path d="M0,0 L10,5 L0,10 z" fill="#2E5F45" />
              </marker>
            </defs>
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  className="flex w-full flex-shrink-0 items-center justify-center gap-1.5 bg-[#3A7D5A] py-[5%]"
                >
                  <Icon className="h-[clamp(10px,2.2vw,15px)] w-[clamp(10px,2.2vw,15px)] text-white" />
                  <span
                    className="whitespace-nowrap font-heading font-bold uppercase tracking-wide text-white"
                    style={{ fontSize: "clamp(8.5px, 2vw, 13px)" }}
                  >
                    {s.label}
                  </span>
                </div>
                ...
                {/* Description */}
                <div className="flex flex-1 items-start justify-center px-[12%] pt-[6%]">
                  <p
                    className="text-center font-body leading-snug text-[#1C2321]"
                    style={{ fontSize: "clamp(7.5px, 1.7vw, 11.5px)" }}
                  >
                    {s.desc}
                  </p>
                </div>

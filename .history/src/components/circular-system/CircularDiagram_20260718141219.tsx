...
const TOOLTIP_CFG = {
  top:    { from: { y: 10 }, cls: "bottom-[calc(100%+10px)] left-1/2 -translate-x-1/2" },
  bottom: { from: { y: -10 }, cls: "top-[calc(100%+10px)] left-1/2 -translate-x-1/2" },
  left:   { from: { x: 10 }, cls: "right-[calc(100%+10px)] top-1/2 -translate-y-1/2" },
  right:  { from: { x: -10 }, cls: "left-[calc(100%+10px)] top-1/2 -translate-y-1/2" },
};
...
export default function CircularDiagram() {
  ...
  const nodeRefs    = useRef<Record<StageKey, HTMLButtonElement | null>>({ milking: null, fertilizer: null, crop: null, milling: null });
  const tooltipRefs = useRef<Record<StageKey, HTMLDivElement | null>>({ milking: null, fertilizer: null, crop: null, milling: null });
  const spokeRefs   = useRef<Record<StageKey, SVGLineElement | null>>({ milking: null, fertilizer: null, crop: null, milling: null });
  ...
  const showTooltip = useCallback((key: StageKey) => {
    ...
    gsap.fromTo(el,
      ...
      { opacity: 1, x: 0, y: 0, pointerEvents: "auto", duration: 0.3, ease: "power3.out" }
    );
  }, []);
  ...
  const hideTooltip = useCallback((key: StageKey) => {
    ...
    if (!el) return;
    const { from } = TOOLTIP_CFG[STAGE_DATA[key].tooltipSide];
    gsap.to(el, { opacity: 0, ...from, pointerEvents: "none", duration: 0.18, ease: "power2.in" });
  }, []);
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      onEnter() {
        ...
        // Stagger chips in
        gsap.fromTo(chipRefs.current,
          { opacity: 0, scale: 0.85 },
          { opacity: 1, scale: 1, duration: 0.5, stagger: 0.1, ease: "back.out(1.4)", delay: 0.4 }
        );

        applyParticle(90);
        gsap.set(nodeRefs.current.milking, { scale: 1.08 });
        gsap.set(spokeRefs.current.milking, { opacity: 0.5 });
        showTooltip("milking");
        startOrbit();
      },
    });

    return () => { st.kill(); tweenRef.current?.kill(); };
  ...
  const handlePlayPause = useCallback(() => {
    ...
    setIsPlaying(next);
    if (next) startOrbit(); else pauseOrbit();
  }, [startOrbit, pauseOrbit]);

  const handleNodeClick = useCallback((key: StageKey) => {
      ...
      <div className="container-pad mx-auto max-w-5xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-[15px] font-body text-[#6B6259] max-w-md mx-auto leading-relaxed">
            Four connected stages. Zero waste. A closed loop where every resource stays in use.
          </p>
        </div>
        ...
        >
          ...
          {CHIPS.map((chip, i) => (
            <div
              ...
              style={{ opacity: 0 }}
              className={`absolute ${chip.corner} z-30 bg-white/80 backdrop-blur-sm border border-neutral-200/80 rounded-xl px-3 py-2 shadow-sm pointer-events-none`}
            >
              <p className="text-[9px] font-body font-semibold uppercase tracking-widest text-[#6B6259]">{chip.label}</p>
              <p className="text-[15px] font-heading font-bold text-[#1C2321] leading-tight">{chip.value}</p>
            </div>
          ))}
          ...
          >
            {/* Outer ghost ring */}
            <circle cx={CX} cy={CY} r={RADIUS + 28} fill="none" stroke="#3A7D5A" strokeWidth="1" opacity="0.07" strokeDasharray="2 10" />
            {/* Inner ghost ring */}
            <circle cx={CX} cy={CY} r={RADIUS - 32} fill="none" stroke="#3A7D5A" strokeWidth="1" opacity="0.07" strokeDasharray="2 10" />
            ...
            {STAGES.map((key) => {
              ...
              return (
                <line
                  key={key}
                  ref={(el) => { spokeRefs.current[key] = el; }}
                  x1={CX} y1={CY}
                  x2={ex} y2={ey}
                  stroke="#3A7D5A"
                  strokeWidth="1"
            ...
            {/* Main orbit ring */}
            <circle cx={CX} cy={CY} r={RADIUS} fill="none" stroke="#E5E5E5" strokeWidth="2" strokeDasharray="4 8" />
            <circle cx={CX} cy={CY} r={RADIUS} fill="none" stroke="#3A7D5A" strokeWidth="1.5" opacity="0.15" />

            {/* Particle */}
            <circle ref={particleRef} cx={CX} cy={CY + RADIUS} r="7"  fill="#3A7D5A" />
            <circle ref={pulseRef}    cx={CX} cy={CY + RADIUS} r="16" fill="none" stroke="#3A7D5A" strokeWidth="2" opacity="0.3" className="animate-ping" />
          </svg>
          ...
          <div className="absolute inset-0 m-auto h-[140px] w-[140px] rounded-full bg-[#1C2321] flex flex-col items-center justify-center text-center p-3 shadow-lg border border-[#3A7D5A]/30 z-20 pointer-events-none">
            <span className="text-[9px] font-bold font-body text-[#6DBE8C] tracking-wider uppercase mb-0.5">Ingaju Farms</span>
            <p className="text-[11px] font-heading font-bold text-white leading-tight">Circular<br />System</p>
              ...
              >
                <div className="w-full h-full rounded-[14px] overflow-hidden relative">
                  <Image
                    ...
                    priority
                  />
                  <div className={`absolute inset-0 rounded-[14px] transition-opacity duration-300 ${activeStage === key ? "opacity-0" : "opacity-20 bg-white group-hover:opacity-0"}`} />
                </div>
                ...
                >
                  <p className="text-[11px] font-heading font-bold text-[#1C2321] mb-1">{s.title}</p>
                  <p className="text-[10px] font-body text-[#6B6259] leading-snug">{s.shortDesc}</p>
                  <span className="mt-2 block text-[9px] font-mono font-bold text-[#3A7D5A] uppercase tracking-wider">{s.metric}</span>
                </div>
              </button>
            );
          })}

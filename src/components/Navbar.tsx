...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="container-pad flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col">
            <span
              className={`font-heading text-xl font-bold leading-none tracking-tight transition-colors duration-300 ${
                scrolled ? "text-[#1C2321]" : "text-white"
              }`}
            >
              Ingaju <span className="text-[#D07A53]">Farms</span>
            </span>
            <span
              className={`text-[9px] font-body font-semibold uppercase tracking-widest transition-colors duration-300 mt-0.5 ${
                scrolled ? "text-[#2E4F41]" : "text-[#6DBE8C]"
              }`}
            >
              Organic & Circular
            </span>
        ...
        <nav className="hidden items-center gap-8 lg:flex">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative py-1 text-sm font-body font-medium transition-colors duration-300 group ${
                scrolled
        ...
        <button
          ...
          className={`lg:hidden rounded-full p-2 transition-colors duration-300 hover:bg-black/5 ${
            scrolled ? "text-[#1C2321]" : "text-white"
          }`}
      ...
      {/* Mobile menu overlay */}
      <div
        className={`absolute top-full left-0 w-full bg-[#1C2321]/95 backdrop-blur-md border-b border-white/10 transition-all duration-300 origin-top shadow-xl ${
          menuOpen
        ...
        <nav className="flex flex-col gap-5 px-6 py-8">
          {navLinks.map((link) => (
            <Link
              ...
              className="text-base font-body font-medium text-white/80 transition-colors hover:text-white flex items-center justify-between border-b border-white/5 pb-2"
            >
              <span>{link.label}</span>
              <ArrowRight className="h-4 w-4 text-[#6DBE8C]" />
            </Link>
          ))}
          <Link
            href="/contact"
